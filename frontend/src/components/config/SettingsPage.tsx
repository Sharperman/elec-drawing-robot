/**
 * 设置页 - LLM 供应商管理 + AutoCAD 设置
 * 从 App.tsx 拆分，原 ~350 行内联组件
 */
...
const LLMEditorModal: React.FC<LLMEditorProps> = ({ provider, onClose, onSaved }) => {
  ...
  const inputCls =
    'w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-400 transition-all';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-md p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-base font-semibold text-gray-100 mb-4">{isEdit ? '编辑供应商' : '添加供应商'}</h3>
...
const SettingsPage: React.FC = () => {
  ...
  const handleSyncEnv = async () => {
    setSyncing(true);
    try {
      await apiClient.post('/api/llm/env');
    } catch {
      /* ignore */
    }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950 p-6 overflow-y-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={handleSyncEnv} disabled={syncing} className="px-3 py-2 rounded-lg bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs transition-colors disabled:opacity-50">
              {syncing ? '已同步 ✓' : '同步到 .env'}
            </button>
            <button
              onClick={() => {
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-0.5 text-xs text-gray-500">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-mono text-gray-400">{visibleKeys.has(p.id) ? p.api_key.replace(/\*/g, '\u2022') : maskKey(p.api_key)}</span>
                ...
                {testResults[p.id] && (
                  <div className={`mt-2 p-2.5 rounded-lg text-xs ${testResults[p.id].status === 'ok' ? 'bg-green-500/10 border border-green-500/20 text-green-400' : 'bg-red-500/10 border border-red-500/20 text-red-400'}`}>
                    {testResults[p.id].status === 'ok' ? (
                      <>
                        连接成功 — {testResults[p.id].elapsed_ms}ms — 模型: {testResults[p.id].model}
                        {testResults[p.id].response && <div className="text-green-500/70 mt-0.5">响应: {testResults[p.id].response}</div>}
                      </>
                    ) : (
      ...
      <div className="border-t border-gray-800 pt-5">
        ...
        <div className="max-w-lg space-y-4">
          ...
          <div className="flex justify-end pt-1">
            <button onClick={handleSave} className="px-5 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium transition-all shadow-sm shadow-blue-500/25 active:scale-95">
              {saved ? '已保存 ✓' : '保存设置'}
            </button>
          </div>
        </div>
      ...
      {/* 编辑弹窗 */}
      {editorOpen && <LLMEditorModal provider={editingProvider} onClose={() => { setEditorOpen(false); setEditingProvider(null); }} onSaved={() => { setEditorOpen(false); setEditingProvider(null); fetchProviders(); }} />}
    </div>
  );
};
